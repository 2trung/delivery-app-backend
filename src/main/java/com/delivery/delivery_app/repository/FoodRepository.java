...
public interface FoodRepository extends JpaRepository<Food, String> {
    ...
    @Query("SELECT new com.delivery.delivery_app.dto.food.FoodResponse(f.id, f.name, f.image, f.price, f.oldPrice, f.likeCount, f.orderCount, r.id, r.name, r.address, r.latitude, r.longitude) FROM Food f JOIN f.foodCategory fc JOIN fc.restaurant r WHERE LOWER(f.name) LIKE LOWER(CONCAT('%', :keyword, '%'))")
    Page<FoodResponse> searchByNameLikeIgnoreCase(@Param("keyword") String keyword, Pageable pageable);
}
